...
namespace IOOP
{
    public partial class ManagerTablesPage : Form
    {
        ...
        private void ManagerTablesPage_Load(object sender, EventArgs e)
        {
            ...
            con.Open();
            SqlCommand cmd = new SqlCommand("SELECT * FROM Reservation WHERE ReservationStatus = 'APPROVED' ORDER BY ReservedDate, StartTime", con);
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (dataGridViewUpcoming.SelectedRows.Count > 0)
                {
                    ...
                    cbbTime.Text = selectedRow.Cells["StartTime"].Value.ToString();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                SqlCommand cmdCheckAvailability = new SqlCommand("SELECT COUNT(*) FROM Reservation WHERE PlaceID = @PlaceID AND ReservedDate = @ReservedDate AND (@StartTime < EndTime AND @EndTime > StartTime) AND ReservationStatus = 'APPROVED'", con);
                ...
                cmdCheckAvailability.Parameters.AddWithValue("@StartTime", startTime);
                ...
                SqlCommand cmd = new SqlCommand("INSERT INTO Reservation (ReservationID, CustomerID, CustomerPax, PlaceID, PlaceName, ReservedDate, StartTime, EndTime, Duration, PlaceSpecialInstructions) VALUES (@ReservationID, @CustomerID, @CustomerPax, @PlaceID, @PlaceName, @ReservedDate, @StartTime, @EndTime, @Duration, @PlaceSpecialInstructions)", con);
                ...
                cmd.Parameters.AddWithValue("@StartTime", cbbTime.Text);
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                SqlCommand cmdCheckAvailability = new SqlCommand("SELECT COUNT(*) FROM Reservation WHERE PlaceID = @PlaceID AND ReservedDate = @ReservedDate AND (@StartTime < EndTime AND @EndTime > StartTime) AND ReservationID != @ReservationID", con);
                cmdCheckAvailability.Parameters.AddWithValue("@PlaceID", placeID);
                cmdCheckAvailability.Parameters.AddWithValue("@ReservedDate", reservedDate);
                cmdCheckAvailability.Parameters.AddWithValue("@StartTime", startTime);
                cmdCheckAvailability.Parameters.AddWithValue("@EndTime", endTime);
                ...
                SqlCommand cmd = new SqlCommand("UPDATE Reservation SET CustomerPax=@CustomerPax, PlaceID=@PlaceID, PlaceName=@PlaceName, ReservedDate=@ReservedDate, StartTime=@StartTime, EndTime=@EndTime, Duration=@Duration, PlaceSpecialInstructions=@PlaceSpecialInstructions WHERE ReservationID=@ReservationID", con);
                cmd.Parameters.AddWithValue("@ReservationID", reservationID);
                cmd.Parameters.AddWithValue("@CustomerPax", txtPax.Text);
                cmd.Parameters.AddWithValue("@PlaceID", cbbPlaceID.Text);
                cmd.Parameters.AddWithValue("@PlaceName", placeName);
