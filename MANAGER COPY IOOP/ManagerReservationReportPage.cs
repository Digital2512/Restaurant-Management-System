    ...
    public partial class ManagerReservationReportPage : Form
    {
        ...
        private void LoadReservationReport()
        {
            try
            {
                using (SqlConnection con = new SqlConnection(connetionString))
                {
                    ...
                    query.Append("SELECT R.ReservationID, R.PlaceID, R.CustomerID, R.CustomerPax, R.ReservedDate, R.StartTime, R.EndTime, POR.EventType ");
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                con.Open();
                SqlCommand cmd = new SqlCommand("SELECT R.ReservationID, R.PlaceID, R.CustomerID, R.CustomerPax, R.ReservedDate, R.StartTime, R.EndTime, POR.EventType, " +
                            "FROM Reservation AS R JOIN PlaceOfReservation AS POR ON R.PlaceID = POR.PlaceID " +
                            "WHERE R.ReservationID LIKE @SearchInput OR R.PlaceID LIKE @SearchInput OR R.CustomerID LIKE @SearchInput OR R.CustomerPax LIKE @SearchInput OR R.ReservedDate LIKE @SearchInput OR R.StartTime LIKE @SearchInput OR R.EndTime LIKE @SearchInput OR POR.EventType LIKE @SearchInput OR O.Tax LIKE @SearchInput", con);
